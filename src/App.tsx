...
// Add an interface for transaction history
...
// Add interface for airdrop creation
interface AirdropCreationParams {
  tokenAmount: string
  recipientsList: string  // CSV format: "address,amount\naddress2,amount2"
  isVesting: boolean
  vestingPeriod?: number  // in seconds
  vestingStart?: number   // timestamp
}
...
function WalletContent() {
  ...
  // Add state for airdrop creation
  ...
  useEffect(() => {
    ...
    // Only refresh every 30 seconds as a backup
  ...
  // Add useEffect to fetch price periodically
  useEffect(() => {
    fetchSolPrice()
    const intervalId = setInterval(fetchSolPrice, 60000) // Update every minute
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setTransactions(prev => [{
        ...
        timestamp: Date.now(),
        signature
      }, ...prev].slice(0, 5)) // Keep last 5 transactions
  ...
  // Update fetchAirdropDetails function
  ...
  // Update claim function with correct SDK method
  const claimAirdrop = async () => {
    ...
    try {
      ...
      // Use createStream method from SDK for claiming
      const { transaction } = await streamflow.createStream({
        id: airdropDetails.id,
        recipient: publicKey.toString(),
        amount: airdropDetails.userAmount,
        mint: 'SOL'
      })
  ...
  // Update createAirdrop function to use correct SDK method
